...
% 2: bulk material with in-plane uniaxial anisotropy ("material" in short)

% coordinate system:
% x: in-plane, perpendicular to axis of material
% y: in-plane, parallel to axis of material
% z: through-plane

% code running time: ~1 s per frequency point



% define filenames for the data to be analyzed
FileNames_data = '103_Mylar_machine_vertical_12um_145mV-100k-100Hz_40P_20X_0p65pump_0p83probe';
...
% ff = logspace(log10(100e3),log10(1000),10)';  % modulation frequency,% (Hz) (vertical
ff = logspace(log10(100e3),log10(100),10)';  % modulation frequency, (Hz)%horizontal
w_rms = 11.3e-6*0.25; % rms beam radii, (m)
r_0 = 12.6e-6*0.25; % beam offset , (m)
 % phi = deg2rad(90); % orientation angle of beam offset relative to
 % x-axis%vertical %machine
phi = deg2rad(0); % orientation angle of beam offset relative to x-axis %horizontal %transverse
% phi = 0 for beam offset perpendicular to axis of material (y-axis)
% phi = pi/2 for beam offset parallel to axis of material (y-axis)

% thermal properties of material
sigma_2_x = 0.3; % thermal conductivity perpendicular to draw direction
sigma_2_y = 0.5; % thermal conductivity along draw direction
sigma_2_z = sigma_2_x;
capac_2 = 1.59e6;  % volumetric heat capacity
rho_2 = 1380;   % density, (kg/m^3)

% elastic and thermal expansion properties of material
% in-plane uniaxial anisotropy
alphaT_vertical = 70e-6; % linear CTE penpendicular to axis of material, (K^(-1))
alphaT_parallel = 60e-6; % linear CTE parallel to axis of material, (K^(-1))
...
% laser power
incident_pump = 0.65e-3;     % avarage power of digital power (square wave) pump before lens (W), set 10 mW
V_sum =V_SUM_avg; % detector sum voltage, (V)
n_al=2.9;              % for 780 nm from Mathewson and Myers
k_al=8.2; 
lens_transmittance = 0.82; % transmittance of lens, 5x
...
L_1 = 80e-9; % thickness, (m)
sigma_1 = 109; % thermal conductivity
capac_1 = 2.42e6;
...
betay_2 = 2*C13_0_2*alphaT_vertical + C33_0_2*alphaT_parallel;
betaz_2 = betax_2;
C11_2 = C11_0_2;
C22_2 = C33_0_2;
C33_2 = C11_0_2;
...
%% calculation of surface displacement
...
for i_fr = 1:length(ff)
    ...
    for i_p = 1:n_p
        ...
        for i_psi = 1:n_psi
            ...
            zeta_2 = sqrt(qn2_2 + k^2*sigma_2_x/sigma_2_z + xi^2*sigma_2_y/sigma_2_z);
...
%% calculation of probe beam deflection angle
...
for i_fr = 1:length(ff)
    ...
    for i_p = 1:n_p
        ...
        for i_psi = 1:n_psi
            psi = ppsi(i_psi);
            g_psi_phi = -cos(psi-phi)*sin(pr0*cos(psi-phi)) - cos(psi+phi)*sin(pr0*cos(psi+phi));
    ...
    PBD_angle(i_fr,1) = 1/pi*C_probe*simpson_inte(Ip2,d_p);
end
...
% fit to polynomial
p=polyfit(log(ff),out_of_phase_PBD_signal,2);
fmax = exp(-p(2)/2/p(1));
disp('frequency of maximum of out-of-phase');
disp(fmax);
p=polyfit(log(ff),log(ratio_PBD_signal),1);
ratio_at_fmax = exp(polyval(p,log(fmax)));
disp('ratio at that frequency');
disp(ratio_at_fmax);
...
semilogx(Fexp, Vin_exp, 'ro-','linewidth',1.5);
hold on;
semilogx(Fexp, Vout_exp, 'rx--','linewidth',1.5);hold on
box on; axis tight;
...
loglog(Fexp, ratio, 'ro-','linewidth',1.5); hold on
box on; axis tight;
set(gca,'linewidth',1.5,'fontsize',16,'fontname','Times New Roman');
xlabel('f (Hz)');
ylabel('ratio');

%title('0 degree, alpha_x=167,alpha_y=75 ppm/K')
%% functions
